...
```{r}
# get predictions
predictions <- predict(rf_model, test_data, type="prob")[, '1']
...
roc_plot <- ggplot(df_for_plot, aes(m = M, d = D)) +
  geom_roc() +
  style_roc() +
  geom_text(aes(x = 0.5, y = .1, label = paste("AUC =", round(auc_value, 4))), 
            size = 5)
...
calibration_data <- data.frame(disease = actual_values, 
                               predicted = predictions)

# Fit logistic regression model for calibration with x=TRUE and y=TRUE
...
feature_importance <- importance_scores[order(importance_scores[, "MeanDecreaseGini"], decreasing = TRUE),]

# Plotting feature importance
varImpPlot(rf_model, type=1, main="Mean Decrease Gini Importance")
